...
router.openapi(createRoute({
  ...
  // 查询 user
  const user = await db.query.user.findFirst({ where: eq(user.name, name) })
  if (!user) return c.json({ message: `用户 ${name} 不存在` }, 404)
  // 查询 user 的关注者(粉丝)
  const followers = await db.select().from(follow_table)
    .leftJoin(user, eq(follow_table.follower_id, user.id))
...
router.openapi(
  createRoute({
    ...
    const [{ name }, { offset, limit }] = [c.req.valid("param"), c.req.valid("query")]
    // 查询 user
    const q_user = await db.query.user.findFirst({ where: eq(user.name, name) })
...
router.openapi(
  ...
  - 注意 people 可能是 user, 也可能是 group`,
    ...
    const followings = await db.select().from(follow_table)
      .leftJoin(user, eq(follow_table.target_id, user.id))
  ...
  createRoute({
    ...
    const followings = await db.select({
      ...
      is_following: sql<boolean>`EXISTS (
        SELECT 1 FROM ${follow_table}
        WHERE ${follow_table.follower_id} = ${auth_user.id}
        AND (${follow_table.target_id} = ${user.id} or ${follow_table.target_id} = ${group_table.id})
      )`.as('is_following'),
      is_following_me: sql<boolean>`EXISTS (
        SELECT 1 FROM ${follow_table}
        WHERE ${follow_table.follower_id} = ${user.id}
