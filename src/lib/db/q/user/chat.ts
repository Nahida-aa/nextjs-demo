...
// import { chat_table, link_chat_user } from "~/lib/db/schema/message";
...
// ) {
//   try {
//     const latestSender_table = aliasedTable(user, "latestSender_table")
//     // const groupMembers_table = aliasedTable(user, "groupMembers_table");

//     const chatsQ = db.select({
//       chat: chat_table,
//       is_pinned: link_chat_user.is_pinned,
//       target_user: {
//         id: user.id,
//         name: user.name,
...
//           )
//           from ${link_chat_user}
//           left join ${user} on ${link_chat_user.user_id} = ${user.id}
//           where ${link_chat_user.chat_id} = ${chat_table.id}
//           limit 4
//         )`.as('members')
//       },
//       latest_sender: {
...
//       }
//     }).from(chat_table)
//       .leftJoin(link_chat_user, eq(chat_table.id, link_chat_user.chat_id))
//       .leftJoin(user, eq(link_chat_user.target_user_id, user.id))
//       .leftJoin(group_table, eq(link_chat_user.target_group_id, group_table.id))
//       .leftJoin(latestSender_table, eq(chat_table.latest_sender_id, latestSender_table.id))
//       .where(eq(link_chat_user.user_id, currentUser_id))
...
//         db.select({ chat_id: link_chat_user.chat_id })
//           .from(link_chat_user)
//           .where(eq(link_chat_user.user_id, user1_id))
//       ),
//       inArray(chat_table.id,
//         db.select({ chat_id: link_chat_user.chat_id })
//           .from(link_chat_user)
//           .where(eq(link_chat_user.user_id, user2_id))
//       )
//     ))
//     .limit(1);
...
//         db.select({ chat_id: link_chat_user.chat_id })
//           .from(link_chat_user)
//           .where(eq(link_chat_user.user_id, user_id))
//       )
//     ))
//     .limit(1);
...
//     if (user_ids) {
//       if (type === 'group') {
//         await tx.insert(link_chat_user).values(
...
//       eq(link_chat_user.user_id, user_id),
//       target_type === 'user'
//         ? eq(link_chat_user.target_user_id, target_id)
//         : eq(link_chat_user.target_group_id, target_id)
//     ))
//     .limit(1);
