...
// import { chat_table, link_chat_user } from "../../schema/message";
// import { user, User, user } from "../../schema/user";
...
//   const usersQuery = db.select().from(user)
//     .where(inArray(user.id, friendIds))
...
//   }).from(user)
//     .where(and(
//       not(eq(user.id, currentUserId)), // 排除当前用户
