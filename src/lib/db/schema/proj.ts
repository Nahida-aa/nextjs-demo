...
export const proj_table = pgTable("Proj", {
  ...
  owner_id: text().notNull(), // user 或 group 的 ID
  creator_id: text().notNull().references(() => user.id), // 创建者的用户 ID
...
export const projectMember_table = pgTable("ProjectMember", {
  id: pgNanoid(),
  project_id: text().notNull().references(() => proj_table.id), // 外键引用 proj 表
  member_id: text().notNull().references(() => user.id), // 成员 ID
